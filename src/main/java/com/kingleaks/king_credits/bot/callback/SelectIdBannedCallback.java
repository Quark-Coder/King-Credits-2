...
public class SelectIdBannedCallback implements CallbackQueryHandler{
    private final BotService botService;
    private final StateManagerService stateManager;

    @Override
    public boolean canHandle(String callbackData) {
        return "SELECT_ID_BANNED".equals(callbackData);
    }
    ...
    public void handle(CallbackQuery callbackQuery) {
        ...
        userState.setStatus("WAITING_FOR_SELECT_ID_BANNED");
        stateManager.setUserState(callbackQuery.getFrom().getId(), userState);

        SendMessage sendMessage = new SendMessage();
        sendMessage.setChatId(chatId);
        sendMessage.setText("Введите id забаненного пользователя");
